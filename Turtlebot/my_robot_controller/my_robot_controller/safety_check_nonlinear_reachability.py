...
class DataLoggerNode(Node):
    def __init__(self):
        ...
        self.old_safe_plan = np.zeros((3, 2))  # Assuming 10-step plan with [linear, angular] values
    ...
    def timer_callback(self):
        ...
        R0 = Zonotope(np.array([x,y,np.cos(yaw2),np.sin(yaw2)]).reshape(4,1), np.diag([0.25, 0.25, 0.01,0.01]))
        ...
        # Enforce safety
        Reachable_Set_NL, obstacles, safety_check, plan_safe,GPT_Safety_Flag = self.safety_layer.enforce_safety_nonlinear(R0,plan,self.Lidar_readings, -yaw2,current_position)
        #Reachable_Set_NL, obstacles, plan_safe = self.safety_layer.enforce_unsafe_nonlinear(R0,plan,self.Lidar_readings, -yaw2,current_position)
        ...
        plan_safe[0, 0] = np.clip(plan_safe[0, 0], 0, 0.1)

        msg.linear.x = plan_safe[0,0]
        msg.angular.z = plan_safe[0,1]
