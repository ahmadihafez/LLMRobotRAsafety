...
class RobotControllerNode(Node):
    ...
    def odom_callback(self, odo: Odometry):
        ...
        prompt = (
            ...
            "Lidar has 18 beams (-45 to +45), values in meters (3.5 means no obstacle). "
            ...
            "Output exactly in this format: LVel:v1 AVel:w1 "
            "where v1 are linear velocity and w1 are angular velocity. "
    ...
    def parse_chat_response(self, response):
        """
        Parse the ChatGPT response to extract linear and angular velocities.
        Supports both "LVel:[0.1]" and "LVel:0.1" formats, including negative values.
        """
        try:
            # Log raw response
            self.get_logger().info(f"Raw response: {response}")

            # Updated regex to match both positive/negative numbers and lists
            match_lvel = re.search(r'LVel:\s*(\[[^\]]+\]|-?\d+\.?\d*)', response)
            match_avel = re.search(r'AVel:\s*(\[[^\]]+\]|-?\d+\.?\d*)', response)
            ...
            if match_lvel and match_avel:
                ...
                # Convert to list of floats (handle both single values & arrays)
                lvel_values = [float(x.strip()) for x in lvel_str.strip('[]').split(',')]
                avel_values = [float(x.strip()) for x in avel_str.strip('[]').split(',')]

                # Ensure both lists have the same length
                while len(lvel_values) < len(avel_values):
                    lvel_values.append(lvel_values[-1])  # Repeat last value
                while len(avel_values) < len(lvel_values):
                    avel_values.append(avel_values[-1])  # Repeat last value

                # Convert to a 2D NumPy array
                velocities = np.array([lvel_values, avel_values]).T
                return velocities
            else:
                raise ValueError("Invalid response format: Could not find 'LVel' or 'AVel' values.")

        except Exception as e:
            self.get_logger().error(f"Error parsing response: {e}")
            return np.zeros((1, 2))  # Return safe default
